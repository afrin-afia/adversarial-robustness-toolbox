...
def createTrainData():
    ...
    for classname in classes:
        ...
        path= directory+"/fruits/mnistjpg/Training/"+classname+"/"
        img_files= os.listdir(path)         #an array of the names of all files
        for i in img_files:
            finalpath= path+i 
            im = image.load_img(finalpath, target_size=(28,28,3))
            im = image.img_to_array(im)
            
            #im = cv2.imread(finalpath)

            #input needs to be in (channel, width, height) format and with pixel values in range [0,1]
...
def createTestData():
    ...
    for classname in classes:
        ...
        path= directory+"/fruits/mnistjpg/Test/"+classname+"/"
        img_files= os.listdir(path)         #an array of the names of all files
        for i in img_files:
            finalpath= path+i 
            im = image.load_img(finalpath, target_size=(28,28,3))
...
classifier.fit(x_train_array, y_train, nb_epochs=4)
print("TRAINING DONE!")
...
attack_fgsm = FastGradientMethod(classifier, eps=0.2)  
x_test_adv = attack_fgsm.generate(x_test_array)

#Test on adv data
preds = classifier.predict(x_test_adv)
accuracy = np.sum(np.argmax(preds, axis=1) == np.argmax(y_test, axis=1)) / len(y_test)
print("Accuracy on adversarial examples (FGSM): {}%".format(accuracy * 100))


#single image
# Generate the adversarial sample:
path=directory+"/fruits/inputOutput/3.jpg"
im = cv2.imread(path)
...
print("Original label: 0, Predicted label (FGSM): "+ classes[np.argmax(prediction)])

cv2.imwrite(directory+"/fruits/inputOutput/out_fgsm_mnist.jpg", 255*adv_images_df[0][0])

########################
##DeepFool attack-

attack_df= DeepFool(classifier)#ProjectedGradientDescent(classifier, targeted=False, max_iter=10, eps_step=1, eps=5)
x_test_adv = attack_df.generate(x_test_array)

#Test on adv data
preds = classifier.predict(x_test_adv)
accuracy = np.sum(np.argmax(preds, axis=1) == np.argmax(y_test, axis=1)) / len(y_test)
print("Accuracy on adversarial examples (DeepFool): {}%".format(accuracy * 100))
...
org_im_in= np.array(org_im_in)
org_im_in= [org_im_in.astype(np.float32)]
adv_images_df = attack_df.generate(x = np.array(org_im_in))

prediction= classifier.predict(adv_images_df)
print("Original label: 0, Predicted label (DeepFool): "+ classes[np.argmax(prediction)])

cv2.imwrite(directory+"/fruits/inputOutput/out_deepfool_mnist.jpg", 255*adv_images_df[0][0])
